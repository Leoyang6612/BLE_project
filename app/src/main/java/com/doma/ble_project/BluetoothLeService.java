...
public class BluetoothLeService extends Service {
    ...
    public final static UUID UUID_Nordic_UART =
            UUID.fromString(MyGattAttributes.Nordic_UART_UUID);

    // Implements callback methods for GATT events that the app cares about.  For example,
    // connection change and services discovered.
    //當連線狀態發生改變時，系統呼叫我們的 callback 函式中的 onConnectionStateChange 方法。而這個方法會將改變的情況通知 UI 畫面
    //BluetoothGattCallback 物件，它提供幾個方法是系統用來通知我們上層呼叫者的。比如說，連線狀態改變、回報 GATT 的服務列表、回報讀或寫屬性、屬性改變....等等方法，
    // 我們可以 override 這些方法，設計成自己的處理方法。
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                Log.d("TAG", "onCharacteristicRead onCharacteristicChanged:");
    ...
    private void broadcastUpdate(final String action) {
        ...
        sendBroadcast(intent); //透過 API 函式 sendBroadcast( ) 將動作與資料傳給畫面 DeviceControlActivity。
    }
    ...
    public boolean connect(final String address) {
        ...
        //連線只要呼叫 connectGatt 函式，同時提供 callback function (mGattCallback) 給系統
